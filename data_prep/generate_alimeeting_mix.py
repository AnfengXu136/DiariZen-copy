"""Generate AliMeeting mix wavs from individual speaker channels.

Iterates through a root AliMeeting directory and runs SoX to create a
mixed wav file for each meeting that has individual speaker channel wavs.
"""
...
CHANNEL_PATTERN = re.compile(r"(R\d+_M\d+)_[FMN]_SPK(\d+)\.wav$")


def find_speaker_groups(meeting_dir: Path) -> Dict[str, List[Tuple[int, Path]]]:
    """Return mapping of base meeting name to list of (speaker, path)."""
    groups: Dict[str, List[Tuple[int, Path]]] = {}
    for wav_path in meeting_dir.glob("*_SPK*.wav"):
        match = CHANNEL_PATTERN.match(wav_path.name)
        if not match:
            continue
        base, speaker = match.group(1), int(match.group(2))
        groups.setdefault(base, []).append((speaker, wav_path))
    return groups
...
def process_meeting(
    ...
    made = skipped = 0
    groups = find_speaker_groups(meeting_dir)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate AliMeeting mixes from individual speaker wav channels using SoX.",
    )
    parser.add_argument(
        "--input-root",
        type=Path,
        default=Path("/scratch1/anfengxu/public_datasets/diarization/AliMeeting"),
        help="Root directory containing AliMeeting folders.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("/scratch1/anfengxu/public_datasets/diarization/AliMeeting-IHM-mix"),
        help="Directory where mixed wavs will be written (mirrors input structure).",
    )
    parser.add_argument(
        "--min-channels",
        type=int,
        default=1,
        help="Minimum number of speaker channels required to build a mix.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
